...
public class PoetryDecoder {
	...
	public static String decode(String hex) {
		...
		return formatPoem(findWordsOfLength(toEnglishLetters(hex), 4),
				findWordsOfLength(toEnglishLetters(hex), 5),
				findWordsOfLength(toEnglishLetters(hex), 6));
	...
	public static String toEnglishLetters(String hex) {
		...
		for (int i = 0; i < hex.length(); i += 2) {
			...
			if (((!(toDecimal >= 97)) || (!(toDecimal <= 122)))
					&& ((!(toDecimal >= 65)) || (!(toDecimal <= 90)))) {

				int modNumber = (toDecimal % 26) + 97; // Modded String
														// calculation
	...
	public static String formatPoem(String[] shortest, String[] medium,
		...
		String megaString = ""; // Recieved help from TA Kristine on getting
								// longest array
		...
		if (longest == null) { // Recieved help from TA Kevin on if a array has nothing in it
								
			lengthL = -1;
		} else
			lengthL = longest.length;
		...
		for (int i = 0; i < biggest; i++) {
			...
			else if ((i) < lengthL) { // Adds it to String if less than the length of longest array
										

				if (longest[i] != null) {

					if ((i + 1) == biggest - 1) {   //If last in the array, don't put a space.
						megaString += longest[i];

					}

					else
						megaString += longest[i] + " "; //If not, add a space.
